...
# the shift network
class _ShiftingNet_my(nn.Module):
    expansion = 1
...
class _ShiftingNet(nn.Module):
    ...
    def __init__(self,batchNorm=True):
        ...
        # due to our is a single v
        self.predict_flow6 = predict_flow(1024)
        self.predict_flow5 = predict_flow(1026-1)
        self.predict_flow4 = predict_flow(770-1)
...
# the shift network
class _ShiftingNet2(nn.Module):
    expansion = 1

    def __init__(self,batchNorm=True):
        super(_ShiftingNet2,self).__init__()
...
def ShiftingNet_init_my(data=None):
 
    #Args:
    #    data : pretrained weights of the network. will create a new one if not set
    #"""
    model = _ShiftingNet_my(batchNorm=False)
    if data is not None:
        model.load_state_dict(data['state_dict'])
    return model

def ShiftingNet_init2(data=None):
 
    #Args:
    #    data : pretrained weights of the network. will create a new one if not set
    #"""
    model = _ShiftingNet2(batchNorm=False)
    if data is not None:
        model.load_state_dict(data['state_dict'])
    return model
