# this is to create training set with 3 image group , one is mask
import cv2
import numpy as np
import os
    ...
    def read_a_batch(self):
        ...
        #for i in range(read_start, read_end):
            ...
            try:
                ...
                #data augmentation
                mask = np.zeros((H_img-Crop_end, W_img))
                masked1 =  np.append(this_pair2[Crop_start:Crop_end,:],mask,axis=0)
                amplifier  = random()
                pair1_piece =   self.gray_scale_augmentation(this_pair1 ,amplifier)
                pair2_piece =   self.gray_scale_augmentation(this_pair2,amplifier)
                pair3_piece =   masked1
